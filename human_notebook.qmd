...
H0 : indépendance entre les variables imc_cat et genre

H1 : dépendance entre les variable imc_cat et genre

<!--% Vérification des conditions d'application -->

```{r}
imc_table <- table(imc_cat = biometry_data$imc_cat, genre = biometry_data$genre)
tabularise(imc_table)
```

```{r test_chi2_1}
  fisher_imc <- fisher.test(imc_table)
  tabularise(fisher_imc)
```
...
Nous rejetton H0 au seuil alpha 5%. Nous pouvons conclure que l'indice de masse grasse (IMG) diffère significativement en fonction de la consommation du nombre de fast_food par semaine ( t-test, df = 52.776, p = 0.0009862 \<\<\<0.05)
