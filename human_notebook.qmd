...
Sur le plan biologique, cela nous indique qu'à mesure que le poids corporel augmente , la graisse s'accumule davantage autour de la taille plutôt que sur les hanches. Ce type de répartition appelée graisse abdominale est connu pour être plus risqué pour la santé et associé à un risque accru de maladies cardiovasculaires et troubles métaboliques . Ainsi ce résultat montre que la répartition de la masse grasse devient plus défavorable à la santé lorsque l'IMC augmente
...
```{r plot_anova2_2}
chart(data = biometry_data, IMG ~ fast_food_rec | genre) +
  geom_boxplot() +
...
```{r test_anova2_2}
mod_anova2 <- aov(IMG ~ genre * fast_food_rec, data = biometry_data)
