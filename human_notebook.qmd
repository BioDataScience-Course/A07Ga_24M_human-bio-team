...
Dans ce tableau, on remarque que plusieurs personnes obèses ont un faible niveau d'activité physique:environ 60% d 'entre elles font peu d'effort physique. Cela pourrait indiquer qu'un manque d'activité physique pourrait être un facteur de risque pour le développement de l'obésite ou du moins que les deux éléments sont liés .De leur coté , les personnes ayant un poids normal sont plus nombreuses à avoir une activité physique élevée . Plus de 40% d'entre elles sont actives , ce qui pourrait faire penser qu'un mode de vie actif joute un rôle dans le maintien d'un poids sain

On observe également que la majorité des personnes en surpoids ont un niveau d'activité faible, ce qui va dans le meme sens que ce qu'on a pu noter avec les personnes obèses

Autre chose importante: L'activité modérée est assez peu frequente dans toutes les catégories . La plupart des personnes ont soit une activité faible , soit une activité élevée, mais peu se situe au milieu.
...
Hypothèse nulle (H₀) :Le niveau d'activité physique et l'indice de masse grasse sont indépendants

Hypothèse alternative (H₁) : Le niveau d'activité physique et l'indice de masse grasse sont dépendants
...
Nous observons dans le tableau de l'anova que la valeur p est très faible et inférieur au seuil alpha de 5% . Du point de vue statistique, nous rejetons H0, nous pouvons dire que le rapport tour de taille sur tour de hanche varie significativement en fonction des groupes d'imc (ANOVA , F=10.1, ddl=3 et 139, valeur P= 4.59\*10E-6)

Sur le plan biologique, cela nous indique qu'à mesure que le poids corporel augmente , la graisse s'accumule davantage autour de la taille plutot que sur les hanches. Ce type de répartition appelée graissse abdominale est connu pour etre plus risqué pour la santé et associé à un risque accru de maladies cardiovasculaires et troubles métaboliques . Ainsi ce résultat montre que la répartition de la masse grasse devient plus défavorable à la santé lorsque l'imc augmente
...
```{r plot_anova2_2}
chart(data = biometry_data, taille_hanche ~ fast_food_rec | genre) +
  geom_boxplot() +
...
H0(hypothèse nulle): il n'ya pas d'effet du genre, ni de la consommation de fast food, ni d'interaction entre les 2 sur l'img

H1:(hypothese alternative):il y'a un effet du genre ou de la consommation de fast food ou une interaction entre les 2 sur l'img

<!--% Vérification des conditions d'application -->

```{r test_anova2_2}
mod_anova2 <- aov(IMG ~ taille_hanche * fast_food_rec, data = biometry_data)
bartlett.test(IMG ~ interaction(genre, fast_food_rec), data = biometry_data)
```
