...
>>>>>>> b1ae4312f7a29ab3f07d2faac6190ac2d34465ba
...
```{r test_anova2_1}
mod_anova2 <- aov(masse_corr ~ genre * sommeil_cat, data = biometry_data)
bartlett.test(masse_corr ~ interaction(genre, sommeil_cat), data = biometry_data)

```

**L**es variances sont homogènes entre les groupes définis par l'interaction `genre * sommeil_cat` (p-value = 0.2187 \> 0.05 )

```{r}
anova_amal <- lm(masse_corr ~ genre * sommeil_cat, data = biometry_data) 
anova(anova_amal) |>
  tabularise()
```
